...
{
	{
		...
		if (_x getVariable "doors" == _doors) then
		{
			...
			if (((_x getVariable "attachedTruck") == _v) && ((_x getVariable "attachedPos") == (_counter)) && (_counter < 0) && !_done) then
			{
				...
				{
					//[_v, _x, [_x_cargo_offset,_counter + 0.25 - _cargo_offset,_zload], [_x_cargo_offset,-6 - _cargo_offset,_zload], 1, _turn] call IL_Move_Attach;
					[_v, _x, [_x_cargo_offset,_counter + 0.25 - _cargo_offset,_zload], [_x_cargo_offset,8,_zload], 1, _turn] call IL_Select_Detach;
				};
				// LCVP
				if ((_obj_type in IL_Supported_Vehicles_LCM3) && (_doors == "B")) then
				{
					//[_v, _x, [_x_cargo_offset,_counter + 0.25 - _cargo_offset,_zload], [_x_cargo_offset,-6 - _cargo_offset,_zload], 1, _turn] call IL_Move_Attach;
					[_v, _x, [_x_cargo_offset,_counter + 0.25 - _cargo_offset,_zload], [_x_cargo_offset,10,_zload], 1, _turn] call IL_Select_Detach;
				};
				// Liberty Ship
				if ((_obj_type in IL_Supported_Vehicles_Cargo_Ship) && (_doors == "B")) then
				{
					...
					[_v, _x, [_x_cargo_offset,_counter + 0.25 + 50,_zload_offset], [_x_cargo_offset,_cargo_offset,_zload], 1, _turn] call IL_Select_Detach;
				};*/
				//C47
				...
				_unloaded = [_v, _x] call IL_Select_Detach;

				if(_unloaded) then {
					_v setVariable["last_cargo", _x, true];
					//_v setVariable["last_cargo", _x];
