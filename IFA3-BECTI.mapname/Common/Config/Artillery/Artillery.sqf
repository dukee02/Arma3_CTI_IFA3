...
if(CTI_SPE_DLC >= 1) then {
		_sid = "";			//it looks like siepatch isn't needed in SPE, if not we add a sidepatch later maybe

		_c pushBack format["%1SPE_GrW278_1", _sid];
		...
		_c pushBack format["%1SPE_M1_81", _sid];
		...
		_c pushBack format["%1SPE_leFH18", _sid];
